...
export const getTrueFalse = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM trueorfalse');
    res.json(rows);
  } catch (error) {
...
export const getTrueFalseById = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM trueorfalse WHERE id_exercise = ?', [req.params.id_exercise]);
    if (rows.length === 0) {
...
export const createTrueFalse = async (req, res) => {
  try {
    ...
    console.log(true_option);
    if (!id_exercise || !true_option || !false_option) {
      return res.status(400).json({ error: 'Datos inválidos' });
    }

    const [result] = await pool.query(
      'INSERT INTO trueorfalse (id_exercise, true_option, false_option) VALUES (?, ?, ?)',
...
export const updateTrueFalse = async (req, res) => {
  try {
    const [result] = await pool.query('UPDATE trueorfalse SET id_exercise = IFNULL(?, id_exercise), true_option = IFNULL(?, true_option), false_option = IFNULL(?, false_option) WHERE id_exercise = ?', [req.body.id_exercise, req.body.true_option, req.body.false_option, req.params.id_exercise]);
...
export const deleteTrueFalse = async (req, res) => {
  try {
    const [results, fields] = await db.execute('CALL DesactivarEjercicio(?)', [req.params.id_exercise]);
    // return results;
  ...
  // try {
  //   const result = await pool.query('DELETE FROM trueorfalse WHERE id_exercise = ?', [req.params.id_exercise]);
  //   if (result.affectedRows === 0) {
